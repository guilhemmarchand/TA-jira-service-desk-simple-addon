...
# connectivity check
def test_jira_connect(account, jira_headers, jira_url, ssl_verify, proxy_dict):
    ...
    try:
        ...
        if response.status_code not in (200, 201, 204):
            raise Exception(
                'JIRA connect verification failed, account=\"{}\", url=\"{}\", HTTP Error=\"{}\", HTTP Response=\"{}\"'.format(account, jira_check_url, response.status_code, response.text))
        else:
